<template>
  <div class="relative overflow-hidden bg-white dark:bg-gray-800 transition-colors duration-300">
    <!-- 背景图片 -->
    ...
    <!-- 内容区域 -->
    <div class="relative p-8 min-h-[240px] flex flex-col justify-end text-center">
      <div class="mb-6">
        ...
        <div class="flex flex-wrap justify-center gap-2 mb-4">
          <span 
            ...
            :key="`cat-${index}`"
            class="px-3 py-1 text-xs font-medium bg-blue-500/20 text-blue-100 rounded-full backdrop-blur-sm"
          >
            {{ category }}
          </span>
          <span 
            ...
            :key="`tag-${index}`"
            class="px-3 py-1 text-xs font-medium bg-purple-500/20 text-purple-100 rounded-full backdrop-blur-sm"
          >
            #{{ tag }}
          </span>
        </div>
        
        <div class="flex flex-wrap justify-center items-center text-white/90 text-sm gap-x-6 gap-y-2">
    ...
    <div class="absolute bottom-0 left-0 w-full h-16 overflow-hidden">
      <!-- 浅色模式波浪 -->
      <svg 
        class="absolute bottom-0 w-full h-full dark:hidden"
        viewBox="0 0 1440 320" 
        preserveAspectRatio="none"
      >
        <defs>
          <linearGradient id="lightWave1" x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%" stop-color="rgba(242, 245, 249, 0.8)" />
            <stop offset="50%" stop-color="rgba(242, 245, 249, 0.9)" />
            <stop offset="100%" stop-color="rgba(242, 245, 249, 0.8)" />
          </linearGradient>
          <linearGradient id="lightWave2" x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%" stop-color="rgba(242, 245, 249, 0.6)" />
            <stop offset="50%" stop-color="rgba(242, 245, 249, 0.7)" />
            <stop offset="100%" stop-color="rgba(242, 245, 249, 0.6)" />
          </linearGradient>
        </defs>
      ...
      <!-- 深色模式波浪 -->
      <svg 
        class="absolute bottom-0 w-full h-full hidden dark:block"
        viewBox="0 0 1440 320" 
        preserveAspectRatio="none"
      >
        <defs>
          <linearGradient id="darkWave1" x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%" stop-color="rgba(4, 7, 17, 0.9)" />
            <stop offset="50%" stop-color="rgba(4, 7, 17, 0.95)" />
            <stop offset="100%" stop-color="rgba(4, 7, 17, 0.9)" />
          </linearGradient>
          <linearGradient id="darkWave2" x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%" stop-color="rgba(4, 7, 17, 0.7)" />
            <stop offset="50%" stop-color="rgba(4, 7, 17, 0.8)" />
            <stop offset="100%" stop-color="rgba(4, 7, 17, 0.7)" />
          </linearGradient>
        </defs>
    ...
    <div class="absolute inset-0 overflow-hidden pointer-events-none">
      <div
        ...
        :style="{
          ...
          width: `${2 + (index % 3)}px`,
          height: `${2 + (index % 3)}px`,
          backgroundColor: `rgba(255, 255, 255, ${0.3 + (index * 0.1) % 0.3})`,
          animationDelay: `${(index * 0.7) % 3}s`,
          animationDuration: `${3 + (index * 0.5) % 2}s`,
          boxShadow: `0 0 ${2 + (index % 2) * 2}px rgba(255, 255, 255, 0.4)`
        }"
      />
    </div>
  </div>
...
// 接收props
const props = withDefaults(defineProps<ArticleInfoProps>(), {
  tags: () => [],
  categories: () => [],
  wordCount: 0
});
...
const readingTime = computed(() => {
  return Math.ceil(props.wordCount / 100);
});
...
onMounted(() => {
  ...
  script.defer = true;
  script.onload = () => {
    const windowWithBusuanzi = window as any;
    if (windowWithBusuanzi.BUSUANZI) {
      windowWithBusuanzi.BUSUANZI.fetch();
    }
  };
